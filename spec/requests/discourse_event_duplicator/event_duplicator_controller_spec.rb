...
RSpec.describe DiscourseEventDuplicator::EventDuplicatorController do
  ...
  describe "#composer_prefill" do
    fab!(:tag) { Fabricate(:tag, name: "grand-prix") }
    fab!(:topic) { Fabricate(:topic, category: category, tags: [tag], title: "Monaco Grand Prix") }
    fab!(:first_post) { Fabricate(:post, topic: topic) }
    fab!(:event) do
    ...
    context "when authorized" do
      ...
      it "returns the title/raw/category/tags for the edited date, without creating anything" do
        get "/event-duplicator/topics/#{topic.id}/composer_prefill.json",
            params: {
              starts_at: "2027-05-24T13:00:00Z",
        ...
        expect(response.status).to eq(200)
        json = response.parsed_body
        expect(json["title"]).to eq("Monaco Grand Prix 2027 (date TBD)")
        expect(json["raw"]).to include('start="2027-05-24 13:00"')
        expect(json["category_id"]).to eq(category.id)
        expect(json["tags"]).to contain_exactly("grand-prix")
        expect(Topic.exists?(title: "Monaco Grand Prix 2027 (date TBD)")).to eq(false)
      end
    end
    ...
    context "when not authorized" do
      ...
      it "returns 403" do
        get "/event-duplicator/topics/#{topic.id}/composer_prefill.json", params: { starts_at: "2027-05-24T13:00:00Z" }
  ...
  describe "#record_duplicate" do
    ...
    context "when authorized" do
      ...
      it "records the tracker entry using the new topic's own event date" do
        new_topic = Fabricate(:topic, category: category, user: member, title: "Monaco Grand Prix 2027")
        new_post = Fabricate(:post, topic: new_topic, user: member)
        Fabricate(:event, post: new_post, original_starts_at: "2027-05-24 13:00")

        post "/event-duplicator/topics/#{topic.id}/record_duplicate.json",
             params: {
        ...
        expect(response.status).to eq(200)
        existing =
          DiscourseEventDuplicator::DuplicationTracker.existing_duplicate_for(
            source_topic: topic,
            target_starts_at: Time.zone.parse("2027-05-24 13:00"),
          )
        expect(existing["topic_id"]).to eq(new_topic.id)
      end

      it "returns 403 when the new topic wasn't authored by the current user" do
        other_user = Fabricate(:user)
        new_topic = Fabricate(:topic, category: category, user: other_user)
        new_post = Fabricate(:post, topic: new_topic, user: other_user)
        Fabricate(:event, post: new_post, original_starts_at: "2027-05-24 13:00")
      ...
      it "returns 404 when the new topic has no event" do
        new_topic = Fabricate(:topic, category: category, user: member)
        Fabricate(:post, topic: new_topic, user: member)
    ...
    context "when not authorized" do
      ...
      it "returns 403" do
        new_topic = Fabricate(:topic, category: category, user: non_member)
